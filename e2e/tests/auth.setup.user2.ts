...
const authFile = path.join(authDir, 'user2.json');
const userDataDir = path.join(__dirname, '../playwright/.browser-data/user2');

/**
 * Authentication Setup - User 2
 *
 * This test opens a browser for you to manually log in via Google OAuth.
 * After you complete the login, it saves your session as User 2.
 *
 * Run with: npm run test:setup:user2
 *
 * Note: This is one of two users needed for friend relationship testing.
 * After setting up both users, run: npm run test:friends
 */
setup('authenticate User 2 via Google OAuth', async ({ }, testInfo) => {
  ...
  console.log('USER 2 LOGIN REQUIRED');
  console.log('='.repeat(60));
  console.log('\n');
  console.log('This is User 2 - the second test account.');
  console.log('Use a DIFFERENT Google account than User 1!');
  console.log('\n');
  ...
  if (isAuthenticated) {
    console.log('\n');
    console.log('='.repeat(60));
    console.log('SUCCESS! User 2 authentication captured.');
    console.log('='.repeat(60));
    console.log('\n');
    console.log(`Session saved to: ${authFile}`);
    console.log('\n');
    console.log('Both users are now set up! Run friend tests with:');
    console.log('  npm run test:friends');
    console.log('\n');
  } else {
    console.log('\n');
    console.log('='.repeat(60));
